...
ggplotly( data_df %>% select(valence, energy, mode, dance, rse_score) %>% pivot_longer(c(valence, energy, mode, dance), names_to = 'spotify', values_to = 'value') %>% ggplot(aes(x = value, y = rse_score)) + geom_point() + geom_smooth() + facet_wrap(~ spotify) )
...
genderplot <- ggplot(data = data_df, aes(x = data_df$demographics.age, y = data_df$rse_score, col = data_df$demographics.gender)) + geom_point() + geom_smooth() + theme_classic() + xlab('RSE Score') + ggtitle('RSE scores per gender') + theme(legend.title = element_blank())
