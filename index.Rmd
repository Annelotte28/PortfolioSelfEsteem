...
ggplotly( data_df %>% select(valence, energy, mode, dance, rse_score) %>% pivot_longer(c(valence, energy, mode, dance), names_to = 'spotify', values_to = 'value') %>% ggplot(aes(x = value, y = rse_score)) + geom_point() + geom_smooth() + facet_wrap(~ spotify) + xlim(c(0,1)) +
  ylim(c(0,30)) + 
  ylab('RSE Score') )
